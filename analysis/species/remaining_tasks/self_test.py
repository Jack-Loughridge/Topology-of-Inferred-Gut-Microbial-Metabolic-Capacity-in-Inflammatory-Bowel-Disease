...
from species_benchmarks_remaining_tasks import (
    ALL_TASKS,
    REMAINING_TASKS,
    RunConfig,
    run,
...
def task_frame(cohort: pd.DataFrame, folder: str) -> pd.DataFrame:
    ...
    if folder == "three_way_nonIBD_UC_CD":
        out = cohort.copy()
        out["label"] = out["diagnosis"]
        return out
    if folder == "nonIBD_vs_UC":
        out = cohort[cohort["diagnosis"].isin(["nonIBD", "UC"])].copy()
        out["label"] = out["diagnosis"]
        return out
    if folder == "nonIBD_vs_CD":
        out = cohort[cohort["diagnosis"].isin(["nonIBD", "CD"])].copy()
        out["label"] = out["diagnosis"]
        return out
    if folder == "CD_vs_UC":
        out = cohort[cohort["diagnosis"].isin(["CD", "UC"])].copy()
        out["label"] = out["diagnosis"]
        return out
    ...
    for task in ALL_TASKS:
        ...
        for repeat in range(1, repeats + 1):
            ...
            for fold, (train_idx, test_idx) in enumerate(
                splitter.split(np.zeros(len(data)), labels, groups=groups), start=1
            ):
...
def write_species(cohort: pd.DataFrame, path: Path) -> None:
    ...
    zero_ids = {"S000_1", "S006_1"}  # one nonIBD and one UC; no participant is removed.
    for _, row in cohort.iterrows():
        values = rng.gamma(shape=1.5, scale=1.0, size=len(features)) + class_offsets[row["diagnosis"]]
        if row["sample_id"] in zero_ids:
            values[:] = 0.0
        output = {"External ID": row["sample_id"]}
        output.update(dict(zip(features, values)))
        rows.append(output)
    # Duplicate two rows to test deterministic duplicate aggregation.
    rows.extend([dict(rows[2]), dict(rows[5])])
    pd.DataFrame(rows).to_excel(path, sheet_name="Sheet1", index=False)


def main() -> None:
    with tempfile.TemporaryDirectory(prefix="species_remaining_selftest_") as tmp:
        root = Path(tmp)
        ...
        cohort = make_cohort()
        write_manifests(cohort, split_dir, repeats=2, folds=2)
        write_species(cohort, species_file)

        config = RunConfig(
            species_file=str(species_file),
            species_sheet="Sheet1",
            ...
            expected_repeats=2,
            expected_folds=2,
            expected_global_zero_profiles=2,
            n_jobs=1,
            logistic_max_iter=4000,
            rf_trees=30,
            xgb_estimators=20,
            make_plots=True,
            save_full_source_models=True,
        )
        ...
        run(config, Args())
        expected_folds = 4 * 3 * 2 * 2
        markers = list(output_dir.glob("*/folds/*/repeat_*/fold_*/FOLD_COMPLETE.json"))
        assert len(markers) == expected_folds, (len(markers), expected_folds)
        ...
        assert set(combined["task_folder"]) == set(REMAINING_TASKS)
        ...
        assert "ovr_auc_nonIBD" in three_way.columns
        assert "ovr_auc_UC" in three_way.columns
        assert "ovr_auc_CD" in three_way.columns
        for task in REMAINING_TASKS:
            excluded = pd.read_csv(output_dir / task / "excluded_species_profiles.csv")
            assert not excluded.empty
            assert (output_dir / task / "full_source_models" / "logistic_l1" / "full_source_model.joblib").exists()
    ...
    print(
        "SELF-TEST PASSED: exact shared task manifests, fixed four-task ordering, explicit global and task-level "
        "zero-profile exclusion, participant-leakage safeguards, train-only CLR/variance/scaling, binary and "
        "multiclass probability alignment and OOF metrics, single XGBoost class weighting, feature stability, "
        "full-source model persistence, final aggregation, and no-refit resume behavior all succeeded."
    )
