...
"""Repeated participant-grouped species-abundance benchmarks for four IBD tasks.
...
The four tasks are run in this fixed order:
  1. non-IBD vs UC vs CD
  2. non-IBD vs UC
  3. non-IBD vs CD
  4. UC vs CD (manifest folder: CD_vs_UC)

The IBD-vs-non-IBD benchmark is handled by the earlier dedicated repository.
"""
...
REMAINING_TASKS = tuple(task.folder for task in ALL_TASKS[1:])
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--tasks", type=parse_csv_tuple, default=REMAINING_TASKS)
...
def config_from_args(args: argparse.Namespace) -> RunConfig:
    ...
    invalid_tasks = sorted(set(tasks) - set(REMAINING_TASKS))
    if invalid_tasks:
        raise ValueError(f"This package runs only the remaining four tasks. Invalid: {invalid_tasks}")
    if len(set(tasks)) != len(tasks):
        raise ValueError("Task list contains duplicates.")
    ordered = tuple(task for task in REMAINING_TASKS if task in set(tasks))
    if ordered != tasks:
        raise ValueError(f"Tasks must preserve the locked order: {REMAINING_TASKS}")
...
def scientific_payload(config: RunConfig) -> dict[str, Any]:
    ...
    payload["task_order"] = list(REMAINING_TASKS)
...
def aggregate_root(config: RunConfig, bundle: dict[str, Any], root: Path) -> None:
    ...
    for task_folder in config.tasks:
        ...
        for filename, target in (
            ...
            frame.insert(0, "task_order", list(REMAINING_TASKS).index(task.folder) + 2)
            target.append(frame)
    if summary_frames:
...
def print_preflight(config: RunConfig, bundle: dict[str, Any]) -> None:
    print("=" * 118)
    print("REPEATED SPECIES BENCHMARKS — REMAINING FOUR TASKS PREFLIGHT")
    print("=" * 118)
    ...
    for index, row in bundle["task_audit"].iterrows():
        print(
            f"  {index + 2}. {row['task']}: {int(row['complete_case_samples'])} / {int(row['source_samples'])} samples, "
    ...
    print("REPEATED SPECIES BENCHMARK REMAINING-TASK PREFLIGHT: PASSED")
    print("=" * 118)
...
def run(config: RunConfig, args: argparse.Namespace) -> Path:
    ...
    print("=" * 118)
    print("ALL REQUESTED SPECIES BENCHMARK TASKS COMPLETE")
    print(f"Output: {root}")
