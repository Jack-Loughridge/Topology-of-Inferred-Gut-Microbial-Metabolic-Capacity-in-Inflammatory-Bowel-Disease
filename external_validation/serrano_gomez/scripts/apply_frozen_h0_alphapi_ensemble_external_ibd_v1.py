...
def latex_metrics_table(results: dict[str, dict[str, Any]], paired: dict[str, Any]) -> str:
    ...
    lines.extend(
        [
            ...
            r"\footnotesize",
            r"Probabilities are the unweighted mean of five corrected outer-training models, each using its own train-only adaptive interval bounds. The v1 representation is the strict frozen-support external analysis. The v2 representation uses external-cohort support normalization and is therefore an exploratory structural replication, not a second strict external validation. AP denotes average precision for IBD.",
            r"\end{minipage}",
            r"\end{table}",
            "",
        ]
